...
class DutyCycledImuUwbNaEkfAlgorithm(BaseLocalizationAlgorithm):
    """
    NLOS-Aware, Duty-Cycled Fused IMU-UWB Adaptive EKF for 2D tag localization.
    ...
    Combines:
      ...
      - NLOS awareness: per-anchor R is inflated by LAMBDA_NLOS whenever that
        anchor's measurement is flagged NLOS, before the usual exponential
        smoothing -- ported from NaAekfAlgorithm's _nlos_mask/_update logic
    ...
    Within each cycle (default 3.0s total), UWB is only fused during the
    final active_window seconds (default 1.0s) -- e.g. with the defaults,
    seconds [0, 2) run IMU-only dead-reckoning, seconds [2, 3) fuse UWB.
    Both the cycle length and the active window are configurable via the
    constructor so they can be tuned without subclassing.

    NLOS gating only matters during the active window: when the window is
    closed, _update receives effective_measurements=None and returns before
    ever touching is_los, so no separate interaction code is needed between
    duty-cycling and NLOS awareness -- the existing early-return handles it.

    Works in three prediction modes depending on what IMU data is available
    this tick, independent of the duty-cycle gate:
        UWB-only  -> constant-velocity prediction + UWB distance AEKF (NLOS-gated)
    ...
    NOTE: this also assumes input_data.is_los exists with the same
    contradictory-but-followed convention as NaAekfAlgorithm: 0 = LOS,
    1 = NLOS, despite the "is_los" name. Unverified against the real
    base_algorithm.py -- same caveat as the standalone NaAekfAlgorithm port.
    """
    ...
    def __init__(self, *args, cycle_length: float = None, active_window: float = None, **kwargs):
        """
        cycle_length  : total duty-cycle period in seconds (default 3.0, matches
    ...
    @property
    def name(self) -> str:
        return "Duty-Cycled IMU-UWB NA-EKF"
    ...
    def update(self, input_data: AlgorithmInput) -> AlgorithmOutput:
        ...
        state, P, Q, R = self._update(state_pred, P_pred, effective_measurements, anchors, Q, R, is_los)
    ...
    def _nlos_mask(self, is_los, n) -> np.ndarray:
        ...
        Ported from NaAekfAlgorithm._nlos_mask. `is_los` is expected to
        follow the same convention as the comment in `update()`: 0 = LOS,
        1 = NLOS, despite the `is_los` name. If it is missing or malformed
        (wrong length against the already duty-cycle/anchor-clamped n), every
        measurement is treated as LOS so the filter degrades gracefully to
        plain AEKF behaviour rather than failing.

        Note n here is the same n = min(len(measurements), len(anchors))
        already computed in _update, not the raw len(anchors) -- a mismatched
        is_los array is checked against that clamped length.
        """
        if is_los is None:
    ...
    def _update(self, state_pred, P_pred, measurements, anchors, Q, R, is_los):
        """
        Joint NLOS-aware adaptive UWB update:
          ...
          - Smooths the resulting R via exponential averaging (AEKF §5.1)
          - Adapts Q  (AEKF §5.2)
          - Applies standard EKF correction

        Falls back to pure prediction (no correction) when no UWB
        measurements are available this tick (either because none arrived,
        or because the duty-cycle window is currently closed).
        """
        ...
        # ── Adaptive R with NLOS gating (AEKF §5.1 + NLOS inflation) ──────
