...
class EskfzuptalgorithmAlgorithm(BaseLocalizationAlgorithm):
    """
    Implementation of My Custom Algorithm
    ...
    def update(self, input_data: AlgorithmInput) -> AlgorithmOutput:
        ...
        if not input_data.imu_data_on or input_data.accel is None:
            ...
            return AlgorithmOutput(
                ...
                Q                   = Q,
                R                   = R,
                extra_data          = {"zupt_triggered": False},
            )

        accel = input_data.accel   # [ax, ay, az]
        gyro  = input_data.gyro   # [gx, gy, gz]

        accel = np.asarray(accel, dtype=float)   # (3,)
        gyro  = np.asarray(gyro,  dtype=float)   # (3,)
