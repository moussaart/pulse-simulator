...
class ImuUwbAdaptiveEkfAlgorithm(BaseLocalizationAlgorithm):
    """
    Fused IMU-UWB Adaptive EKF for 2D tag localization.

    State layout (packed into a single 6-vector, mirroring imu_uwb_aekf):
    ...
    def update(self, input_data: AlgorithmInput) -> AlgorithmOutput:
        ...
        # ── 3. Prediction (UWB-only / IMU-only / ZUPT path) ──────────────
        ...
        # ── 4. UWB adaptive measurement update ───────────────────────────
        state, P, Q, R = self._update(state_pred, P_pred, measurements, anchors, Q, R)

        # ── 5. ZUPT measurement update (velocity -> 0) ───────────────────
        if is_stationary:
    ...
    def _initialise(self, tag):
        ...
        # yaw/gyro_bias are stashed via the packed state, not separate fields,
        # so initial yaw needs to ride along on the first _pack_state call
        self._init_yaw = yaw0
    ...
    def _pack_state(self, ekf_state, yaw, gyro_bias) -> np.ndarray:
        return np.array(
            [ekf_state[0], ekf_state[1], ekf_state[2], ekf_state[3], yaw, gyro_bias],
            dtype=float,
        )
